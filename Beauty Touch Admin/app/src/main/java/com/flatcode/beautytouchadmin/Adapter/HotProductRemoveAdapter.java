...
public class HotProductRemoveAdapter extends RecyclerView.Adapter<HotProductRemoveAdapter.ViewHolder> {

    private final Context mContext;
    private final List<Post> mPost;

    private static ItemProductRemoveBinding binding;

    public HotProductRemoveAdapter(Context context, List<Post> mPost) {
    ...
    @NonNull
    @Override
    public HotProductRemoveAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        binding = ItemProductRemoveBinding.inflate(LayoutInflater.from(mContext), parent, false);
        return new ViewHolder(binding.getRoot());
    }

    @Override
    public void onBindViewHolder(@NonNull final HotProductRemoveAdapter.ViewHolder holder, final int position) {
        ...
        holder.remove.setOnClickListener(view -> FirebaseDatabase.getInstance().getReference().child(DATA.HOT_PRODUCT).child(post.getPostid()).removeValue());
